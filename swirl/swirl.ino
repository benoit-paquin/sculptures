...
const int panelResX = 80;                // Horizontal resolution of one panel.
const int panelResY = 106;                // Vertical resolution of one panel.
...
const int HEIGHT = 106;//                // Total height: 64 pixels.
               ...
               float tiltAngle, uint8_t hueOffset)
{
  ...
  for (int i = 0; i < NUM_STARS; i++) {
    ...
    canvas.fillCircle(screenX*3,screenY*3,1, tft.color565(rr, gg, bb));
  ...
  for (int y = 0; y < HEIGHT; y++) {
    for (int x = 0; x < WIDTH; x++) {
      ...
      if (dist <= blastRadius) {
        ...
        canvas.fillCircle(x*3, y*3, 1, tft.color565(newR, newG, newB));
      }
    }
  }
}
...
void resetScene() {
  ...
  for (int y = 0; y < HEIGHT; y++){
    for (int x = 0; x < WIDTH; x++){
      ...
      canvas.fillCircle(x*3, y*3, 1, 0); // Draw black.
      }
  }
...
void setup(void) {
  ...
  tft.init(240,320);//,SPI_MODE3);           // Init ST7789 320x240
...
void loop() {
  ...
  for (int y = 0; y < HEIGHT; y++) {
    for (int x = 0; x < WIDTH; x++) {
      ...
      canvas.fillCircle(x*3, y*3, 1, tft.color565(r, g, b));
    }
  }
