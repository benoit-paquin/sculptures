...
const int panelResX = 120;                // Horizontal resolution of one panel.
const int panelResY = 120;                // Vertical resolution of one panel.
const int panel_chain = 2;               // Number of panels chained together horizontally.
const int WIDTH  = 80;//panelResX * panel_chain;  // Total width: 64 * 2 = 128 pixels.
const int HEIGHT = 80;//                // Total height: 64 pixels.
...
#define NUM_STARS  50       // Number of stars (points) per swirl.
               ...
               float tiltAngle, uint8_t hueOffset)
{
  ...
  for (int i = 0; i < NUM_STARS; i++) {
    ...
    //canvas.drawPixel(screenX*2,screenY*2, tft.color565(rr, gg, bb));
    canvas.fillCircle(screenX*3,screenY*3,3, tft.color565(rr, gg, bb));
  ...
  for (int y = 0; y < HEIGHT; y++) {
    for (int x = 0; x < WIDTH; x++) {
      ...
      if (dist <= blastRadius) {
        ...
        //canvas.drawPixel(x*2, y*2, tft.color565(newR, newG, newB));
        canvas.fillCircle(x*3, y*3, 3, tft.color565(newR, newG, newB));
      }
    }
  }
}
...
void resetScene() {
  ...
  for (int y = 0; y < HEIGHT; y++){
    for (int x = 0; x < WIDTH; x++){
      ...
      frameBuf[y][x][1] = 0;
      frameBuf[y][x][2] = 0;
      //canvas.drawPixel(x*2, y*2, 0); // Draw black.
      canvas.fillCircle(x*3, y*3, 3, 0); // Draw black.
      }
  }
...
void setup(void) {
  ...
  tft.init(240,240,SPI_MODE3);           // Init ST7789 320x240
...
void loop() {
  ...
  for (int y = 0; y < HEIGHT; y++) {
    for (int x = 0; x < WIDTH; x++) {
      ...
      frameBuf[y][x][1] = g;
      frameBuf[y][x][2] = b;
      //canvas.drawPixel(x*2, y*2, tft.color565(r, g, b));
      canvas.fillCircle(x*3, y*3, 3, tft.color565(r, g, b));
    }
  }
