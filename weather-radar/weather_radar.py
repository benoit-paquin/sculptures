...
def get_station_data(station_param=None):
    ...
    for record in station_file["features"]:
        if record["properties"]["id"] == station.upper():  # If the ID matches
            ...
            station_mode = record["properties"]["rda"]["properties"][
                "volumeCoveragePattern"
            ]  # Mode of the station (e.g. R35, R21)

            # Check the status of the station by looking at the last received time.
            latency_time = datetime.strptime(
                record["properties"]["latency"]["levelTwoLastReceivedTime"],
                "%Y-%m-%dT%H:%M:%S%z",
            )
            ...
            if diff_time.days > 0:  # If greater than 1 day, then it's definitely down
                ...
                if diff_time.seconds < (
                    60 * 10
                ):  # Less than 10 mins, it's probably up!
                    station_status = "Up"
                elif diff_time.seconds > (60 * 10) and diff_time.seconds < (
                    60 * 60
                ):  # Between 10 mins & 60, warning!
...
def get_bounding_coordinates(url):
    ...
    if response:
        ...
        capabilties_dict = xmltodict.parse(
            xml_file.read()
        )  # Parse the XML into a dictionary
        bounding_coordinates = capabilties_dict["WMS_Capabilities"]["Capability"][
            "Layer"
...
def get_radar_images(
    ...
    ########################################
    base_map, base_map_labels, map = get_basemap(
        "coordinate", provider=base_map_layer, zoom=zoom, width=320
    )
    size = map.size
    ...
    if frames == None:
        if station_mode in [
            ...
            "R35",
            "---",
        ]:  # Less frames when in clean air mode.
            num_frames = 5
        else:
            num_frames = 10
    else:
    ...
    for i, image in enumerate(prev_images):
        ...
        extrema = radar.convert(
            "L"
        ).getextrema()  # Extrema reports the min & max colour values.
        ...
        if len(warnings_list) > 0:
            ...
            for warning in warnings_list:
                ...
                combined_warning_annotation.polygon(
                    polygon, fill=fill_colour, outline=stroke_colour
                )
            ...
            if "Tornado Warning" in [
                elem for sublist in warnings_list for elem in sublist
            ]:
                warning_fill_colour = (255, 0, 0, 255)
            elif "Tornado Watch" in [
                elem for sublist in warnings_list for elem in sublist
            ]:
                warning_fill_colour = (255, 174, 0, 255)
            elif "Severe Thunderstorm Warning" in [
                elem for sublist in warnings_list for elem in sublist
            ]:
                warning_fill_colour = (255, 255, 0, 255)
            elif "Severe Thunderstorm Watch" in [
                elem for sublist in warnings_list for elem in sublist
            ]:
            ...
            if len(hazard_list) > 0:
                ...
                for hazard in hazard_list[0]:
                    ...
                    for hazard_points in polygon_hazard:
                        pix_x, pix_y = map.rev_geocode(
                            (hazard_points[0], hazard_points[1])
                        )
                    ...
                    combined_hazard.polygon(
                        polygon_hazard, fill=fill_colour, outline=stroke_colour
                    )
                    pass
        ...
        combined_annotation.ellipse(  # Decorative ring!
            (10, -30, 310, 270), outline=(150, 150, 150, 255), width=7
        )
        # If there's a LOCAL warning, the make the ring thicker
        if len(local_warnings) > 0:
            combined_annotation.ellipse(
                (10, -30, 310, 270), outline=warning_fill_colour, width=15
            )

        # Date & Time
        the_time_local = convert_tz(
            times_datetime[prev_times[i]], "UTC", timeZone
        )  # Convert to local timezone
        datetime_string = the_time_local.strftime("%H:%M %Z")  # Make it into a string
        time_since = (
            datetime.now(pytz.timezone(timeZone)) - the_time_local
        )  # Calculate time since using current time.
        ...
        if len(local_alerts[0]) > 0:
            ...
            for a_hazard_type in unique_hazards:
                ...
                text_length = combined_annotation.textlength(
                    a_hazard_type, font=alert_font
                )
    ...
    if response_hazard:
        ...
        if total_hazards > 0:
            for index, record in enumerate(hazard_json_file["features"]):
                ...
                onset_local_datetime = convert_tz(
                    onset, "UTC", timeZone
                )  # Converted to local datetime
                ends_local_datetime = convert_tz(
                    ends, "UTC", timeZone
                )  # Converted to local datetime
                onset_local = datetime.strftime(
                    onset_local_datetime, "%H:%M (%A %d %B)"
                )  # Remade to string
                ends_local = datetime.strftime(
                    ends_local_datetime, "%H:%M (%A %d %B) %Z"
                )  # Remade to string
        ...
        if warning_json_file["totalFeatures"] > 0:
            for record in warning_json_file["features"]:
                ...
                expiration_datetime = datetime.strptime(
                    record["properties"]["expiration"], "%Y-%m-%dT%H:%M:%S%z"
                )
                expiration = datetime.strftime(
                    expiration_datetime, "%Y-%m-%d, %H:%M %Z"
                )
                ...
                warnings_list.append([warning_type, expiration, polygons_coordinates])
...
def get_times(url):
    ...
    if response:
        ...
        times = capabilties_dict["WMS_Capabilities"]["Capability"]["Layer"]["Layer"][
            "Dimension"
        ]["#text"]
        times_datetime = []
...
def get_basemap(mode, zoom, width, provider="stamen-toner"):
    ...
    if mode == "coordinate":
        ...
        map = geotiler.Map(
            center=(lat_long[1], lat_long[0]), zoom=zoom, size=size, provider=provider
        )
        map_labels = geotiler.Map(
        ...
        map = geotiler.Map(
            extent=(minx, miny, maxx, maxy), zoom=zoom, provider=provider
        )
        map_labels = geotiler.Map(
...
def status_images(
    message,
    background=None,
    background_colour=(100, 100, 100, 200),
    font=fnt_goth_bold,
    ...
    text_bbox = status_annotation.multiline_textbbox(
        (0, 0), message, font=font, spacing=2
    )
    ...
    status_annotation.chord(  # Rounded end
        (x2_pos - 15, y_pos, x2_pos + 15, y2_pos), 270, 90, fill=background_colour
    )
...
while True:
    try:
        ...
        print(f"\n----------------------\n     Local info:\n----------------------")
        local_warnings, local_alerts = get_all_alerts(
            coordinates=(lat_long[1], lat_long[0])
        )

        print(f"\n\n----------------------\n     Greater area:\n----------------------")
        warnings_list, hazard_list = get_all_alerts(
            "Storm",
            "Extreme Wind",
            "High Wind",
            "Gale Warning",
        ...
        time_now = datetime.now(pytz.timezone(timeZone))
        image_with_status = status_images(
            f"⟳ {time_now.strftime('%H:%M')}", latest_image
        )
